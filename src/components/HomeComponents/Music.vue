...
export default {
  ...
  methods: {
    getMusic(id) {
      if(id <= 0) this.music_index = id = 10;
      if(id > 10) this.music_index = id = 1;
      getnotedetail(`/music/getmusic/${id}`).then(res => {
        if (res.data.data.length && !res.data.err) {
          this.music_link = `//music.163.com/outchain/player?type=2&id=${res.data.data[0].music_id}&auto=1&height=66`;
          this.music_index = res.data.data[0].id;
        } else {
          this.$Message.error("暂无音乐!");
        }
      });
    }
  }
