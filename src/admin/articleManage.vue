<template>
 <div class="articleManage">
    ...
    <Page :page-size="pagesize" @on-change="Pagechange" style="margin-top:2rem" :total="count" show-total />
 </div>
</template>
<script>
import {PageSizeChange, PostMessage} from '../components/NetWork/request'
    export default {
        mounted() {
        ...
        methods: {
            ...
            getPage(index) {
                PageSizeChange('/page/getnotePage?page='+index+'&pagesize='+this.pagesize)
                .then(res => {
                    if(res.data.err === 0) {
            ...
            remove (index) {
                PostMessage('/note/articleDelete', {article_id: this.data6[index].article_id})
                .then(res =>{
                  if(res.data.err === 0) {
